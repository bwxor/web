...
export const SecurityContextProvider: React.FC<{children: React.ReactNode}> = ({children}) => {
    const getInitialSecurity = (): SecurityType => {
        return {
            invalidLoginCount: parseInt(sessionStorage.getItem("invalidLoginCount") ?? "0") || 0,
        };
    };
    ...
    useEffect(() => {
        const invalidLoginCount = parseInt(sessionStorage.getItem("invalidLoginCount") ?? "0");
    ...
    const initSecurity = (invalidLoginCount: number) => {
        ...
        sessionStorage.setItem("invalidLoginCount", invalidLoginCount.toString());
    }
